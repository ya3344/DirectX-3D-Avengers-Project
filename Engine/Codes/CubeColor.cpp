...
{
	mVtxInfo = VTX_INFO(sizeof(VTXNORMAL_COL), 24, VTXNORMAL_COL_FVF);
	mIdxInfo = IDX_INFO(sizeof(POLYGON16), 12, D3DFMT_INDEX16);

	VIBuffer::CreateBuffer();

	VTXNORMAL_COL* pVertex = nullptr;
	...
	for (DWORD i = 0; i < mVtxInfo.vtxNum; ++i)
		pVertex[i].color = D3DCOLOR_ARGB(255, 0, 255, 255);
...
void CubeColor::SetColor(DWORD cubeColor)
{
	VTXNORMAL_COL* pVertex = nullptr;
	mVtxInfo.pVB->Lock(0, 0, (void**)&pVertex, 0);
